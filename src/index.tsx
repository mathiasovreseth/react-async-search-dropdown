...
interface AsyncSearchDropdown {
 ...
 disabled?: boolean;
   ...
   */
 renderItem?: (items: Array<any>, i: number, isActive: boolean) => React.ReactNode;
  /**
...
const AsyncSearchDropdown: React.FunctionComponent<AsyncSearchDropdown> = (props: AsyncSearchDropdown) => {
  ...
  const [lastSelectedItem, setLastSelectedItem] = React.useState(props.placeholder);
    ...
    function handleClickItem(item: ItemType, index: number) {
      ...
      */
        if(isSelected2) {
          ...
          const indexToRemove = temp.findIndex(function (tempValue) {
  ...
  function handleSearchChange(e: any) {
    ...
    props.getItems(searchValue).then((result: any)=> {
      setItems(result);
    })
  }
          ...
          {items.map((c: any, i)=> {
            ...
            return (
             <div
               ...
               {!props.defaultListStyling && props.renderItem &&
               props.renderItem(c, i, isActive)}
